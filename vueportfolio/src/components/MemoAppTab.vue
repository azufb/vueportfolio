<template>
    <div>
        <div class='detail'>
            <h3>タグメモ！</h3>
            <button class='btn'>
                <a class='goToApp' href='https://polar-shelf-88279.herokuapp.com/'
                    target='_blank' rel='noopener noreferrer'>
                Go to App <i class='fas fa-caret-square-right'></i>
                </a>
            </button>
            <a class='githubIcon2' href='https://github.com/azufb/MemoAppwithRedux' target='_blank' rel='noopener noreferrer'><i class='fab fa-github fa-2x'></i></a>
        </div>
        <h4>制作のきっかけ</h4>
        <p>
            よくメモアプリを利用するが、ささっとメモしてしまうため、後で見返すと、<br>
            何を意味する数字なのか、何のためにメモしたのか、わからないことがある。<br>
            確かに、一般的なメモアプリであればきちんと目的等書かなければわからない。<br>
            しかし、メモそれぞれを分類できるようにすれば、まだ思い出せるかもしれないと<br>
            考えた。そこで、メモを入力する際に、ジャンル選択をし、その選択したジャンル<br>
            と共にメモを表示することができる。
        </p>
        <h4>使用言語・技術等</h4>
        <p>JavaScript・React.js・Redux・Redux Persist・Heroku</p>
    </div>
</template>